...
class DeepStreamOutput(nn.Layer):
    ...
    def forward(self, x):
        # print(x)
        # {'bbox': var tmp_16 : LOD_TENSOR.shape(1, 8400, 4).dtype(float32).stop_gradient(False), 
        #  'bbox_num': var concat_14.tmp_0 : LOD_TENSOR.shape(1, 1, 8400).dtype(float32).stop_gradient(False)}
        boxes = x['bbox']
