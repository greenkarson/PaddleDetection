...
class DeepStreamOutput(nn.Layer):
    ...
    def forward(self, x):
        # print(x)
        # {'bbox': var tmp_16 : LOD_TENSOR.shape(1, 2125, 4).dtype(float32).stop_gradient(False), 
        # 'bbox_num': var concat_4.tmp_0 : LOD_TENSOR.shape(1, 1, 2125).dtype(float32).stop_gradient(False)}
        boxes = x['bbox']
        x['bbox_num'] = x['bbox_num'].transpose([0, 2, 1])
        return paddle.concat((boxes, x['bbox_num']), axis=-1)
