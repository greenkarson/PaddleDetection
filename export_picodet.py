...
class DeepStreamOutput(nn.Layer):
    ...
    def forward(self, x):
        # print(x)
        # {'bbox': var tmp_16 : LOD_TENSOR.shape(1, 8500, 4).dtype(float32).stop_gradient(False), 
        # 'scores': var concat_4.tmp_0 : LOD_TENSOR.shape(1, 80, 8500).dtype(float32).stop_gradient(False)}
        boxes = x['bbox']
        x['scores'] = x['scores'].transpose([0, 2, 1])
        return paddle.concat((boxes, x['scores']), axis=-1)
...
def picodet_export(FLAGS):
    cfg = load_config(FLAGS.config)
    FLAGS.opt['weights'] = FLAGS.weights
    FLAGS.opt['exclude_nms'] = False
...
def main(FLAGS):
    ...
    print('Opening PicoDet model')

    paddle.set_device('cpu')
    cfg, model = picodet_export(FLAGS)
